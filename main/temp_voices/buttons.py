...
class Patterns(View):
    def __init__(self):
        ...
        async def button_callback(inter: discord.Interaction):
            cursor.execute("INSERT INTO patterns (owner, name, ulimit, bitrate) VALUES (%s, %s, %s, %s)", (inter.user.id, inter.user.voice.channel.name, inter.user.voice.channel.user_limit, inter.user.voice.channel.bitrate,))
