...
async def createproverkachannel(member: discord.Member, category: discord.CategoryChannel):
    ...
    new_channel = await member.guild.create_voice_channel(name=f"Проверка | {member.name}", category=category,
...
async def createsupportchannel(member: discord.Member, category: discord.CategoryChannel):
    ...
    new_channel = await member.guild.create_voice_channel(member.name, category=category, user_limit=2,
...
async def checkchannels(member: discord.Member, before_channel: discord.VoiceChannel,
...
def not_in_voice(inter: discord.Interaction):
    if inter.user.voice is None or inter.user.voice.channel not in inter.channel.category.voice_channels:
        return False
...
async def check_if_admin(inter: discord.Interaction, member: discord.Member, admin: List[int]):
    ...
    if any(has_allowed_role) is True:
        return True
    else:
...
async def mostactivevoice(member: discord.Member):
    ...
    if member.voice:
        ...
        if existing_record:
            ...
            cursor.execute("UPDATE activity SET h24 = %s, d7 = %s, all_time = %s WHERE member = %s",
                           (h24, d7, prev_all_time + time_spent, member.id))
    ...
    if after.channel is not None:
        if after.channel.category_id == cfg['voices']['teammate-search-category-id'] and after.channel.id in channels:
            new_channel = await after.channel.category.create_voice_channel(
                name=f"{after.channel.name[2:]} | {member.name}", user_limit=after.channel.user_limit)
            await member.move_to(new_channel)
            onleave_channels.append(new_channel.id)
...
async def adminsessions(member: discord.Member, after: discord.VoiceState, before: discord.VoiceState):
    if isinstance(member, discord.Member) and not member.bot:
        channel_ids_to_check = [1156875426040389672, 1156875743503061012, 1156875973560647731, 1156876069455015946,
                                1156876268143386644, 1156876561006481428]
        ...
        if not is_in_channels_before and is_in_channels_after:
            ...
            if entry_timestamp_match and exit_timestamp_match:
                ...
                cursor.execute("UPDATE adminsessions SET all_time = %s WHERE admin = %s",
                               (time_difference.total_seconds(), member.id))
            ...
            embed = discord.Embed(title="Завершённая Сессия", description="", color=discord.Colour.blurple(),
                                  timestamp=datetime.datetime.now())
            embed.set_author(name=f"{member.name}",
                             icon_url=f"{member.avatar.url if member.avatar else member.default_avatar.url}")
            ...
            embed.add_field(name="",
                            value=f"- <:time:1157158683881525298> Всего провёл: **{int(hours)}** ч. **{int(minutes)}** мин.\n- <:ShieldDone:1157159384124751923> Админ: {member.mention}\n- <:voice:1157158687157276712> Канал: {before.channel.mention}\n- <:role:1157158688977600574> Роль: {member.roles[-1].mention} {role_icon}",
                            inline=False)
...
async def topadminsessions():
    ...
    embed = discord.Embed(title="Самые активные админы за 24 часа", color=discord.Colour.blurple(),
                          timestamp=datetime.datetime.now())
