...
class NewNameChannel(Modal, title="Изменить название"):           
    new_name = TextInput(label="Новое название", placeholder="Введите новое название вашего канала", style=TextStyle.short, max_length=99)   
    async def on_submit(self, inter: discord.Interaction):       
        await inter.user.voice.channel.edit(name=f"{self.new_name.value}")
        
        em_name = Embed(title="", description=f"Успешно изменено на: **{self.new_name.value}**", color=Colour.dark_embed())
...
class NewLimitChannel(Modal, title="Изменить лимит"):           
    ...
    async def on_submit(self, inter: discord.Interaction):
        try:
            ...
            em_limit = Embed(title="", description=f"Успешно изменено на: **{self.new_limit.value}**", color=Colour.dark_embed()) 
