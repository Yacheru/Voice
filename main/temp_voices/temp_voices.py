...
class Temp(View):
    def __init__(self):
        ...
        async def button_callback(inter: discord.Interaction):
            if not_in_voice(inter=inter) is False:
                ...
                if inter.user.voice.channel.user_limit == 99:
                    await inter.response.send_message("Лимит участников равен **99**. Вы не можете увеличить ещё!",
                                                      ephemeral=True)
                else:
        ...
        async def button_callback(inter: discord.Interaction):
            ...
            if not_owner(inter=inter) is False:
                await inter.response.send_message("Вы не являетесь создателем комнаты!", ephemeral=True)
                return

            else:
                if inter.user.voice.channel.user_limit == 0:
                    await inter.response.send_message("Лимит участников равен **0**. Вы не можете уменьшить ещё!",
                                                      ephemeral=True)
                else:
                    await inter.user.voice.channel.edit(user_limit=inter.user.voice.channel.user_limit - 1)
                    await inter.response.send_message("Успешно изменено!", ephemeral=True)
        ...
        async def button_callback(inter: discord.Interaction):
            ...
            if not_owner(inter=inter) is False:
                ...
                if clicks[inter.user.id] == "first":
                    ...
                    await inter.response.send_message("Ваш канал больше не отображается в списке каналов",
                                                      ephemeral=True)
                    ...
                    await inter.response.send_message("Теперь ваш канал будет отображаться в списке каналов",
                                                      ephemeral=True)
                    clicks[inter.user.id] = "first"
        ...
        async def button_callback(inter: discord.Interaction):
            ...
            if not_owner(inter=inter) is False:
                ...
                if current_time - last_usage_time < 30:
                    await inter.response.send_message(
                        "Пожалуйста, подождите некоторое время перед повторным использованием кнопки.", ephemeral=True)
                else:
        ...
        async def button_callback(inter: discord.Interaction):
            if not_in_voice(inter=inter) is False:
                ...
                participants = [
                    f"{i}) {user.mention} {star}" if result and user.id == result[0] else f"{i}) {user.mention}" for
                    i, user in enumerate(inter.user.voice.channel.members, start=1)]

                em_info = Embed(title=f"Информация о канале - {inter.user.voice.channel.name}",
                                color=Colour.dark_embed(), timestamp=datetime.datetime.now())
                em_info.set_author(name=f"{inter.user.name}",
                                   icon_url=f"{inter.user.avatar.url if inter.user.avatar else inter.user.default_avatar.url}")
                em_info.set_thumbnail(
                    url=f"{inter.user.avatar.url if inter.user.avatar else inter.user.default_avatar.url}")
                em_info.add_field(name="Настройки:",
                                  value=f"Название: **{inter.user.voice.channel.name}**\nЛимит участников: **{len(inter.user.voice.channel.members)}/{inter.user.voice.channel.user_limit if inter.user.voice.channel.user_limit != 0 else 'Без ограничений'}**\nБитрейт: **{inter.user.voice.channel.bitrate}**\nСоздатель: {owner.mention}",
                                  inline=False)
        ...
        async def button_callback(inter: discord.Interaction):
            ...
            if not_owner(inter=inter) is False:
                ...
                if patterns:
                    ...
                    em.add_field(name="Настройки:",
                                 value=f"- Создатель: {owner.mention}\n- Название: **{patterns[2]}**\n- Лимит: **{patterns[3]}**\n- Битрейт: **{patterns[4]}кб/с**")
                    ...
                    em.add_field(name="Вы действительно хотите создать шаблон с такими настройками?",
        ...
        async def button_callback(inter: discord.Interaction):
            em_about_button = Embed(title="", description="### Подробности о каждом взаимодействии",
                                    color=Colour.dark_embed())
            em_about_button.add_field(name="О кнопках:",
                                      value=f"- {arrowdown} / {arrowup} - Данные кнопки изменяют лимит соответствующего канала на 1 в + или -.\n- {show} - Данная кнопка скрывает/показывает ваш канал в списке каналов\n- {unlock} - Данная кнопка открывает/скрывает ваш канал. Его видят, но не могут зайти.\n- {login} - Благодаря этой кнопке вы можете пригласить участника в канал или удалить с него\n- {voice} - Включить или выключить пользователю в вашей комнате микрофон.",
                                      inline=False)
            em_about_button.add_field(name="",
                                      value=f"- {limit} - Поможет вам изменить лимит участников канала до желаемого\n- {edit} - Используйте, если нужно изменить название своего канала\n- {star} - Вы можете передать владение каналом другому участнику.\n- {settings} - Узнать информацию о канале в котором вы находитесь.\n- {folder} - Создать свой шаблон.",
                                      inline=False)
            em_about_button.add_field(name="Меню выбора:",
                                      value="Шаблоны каналов - создание каналов по шаблону. Нельзя изменить кнопками.\nКачество звука - Изменяет bitrate вашего канала. Чем больше - тем лучше!",
                                      inline=False)
            em_about_button.set_author(name=f"{inter.user.name}",
                                       icon_url=f"{inter.user.avatar.url if inter.user.avatar else inter.user.default_avatar.url}")
            em_about_button.set_footer(
                text=f"{inter.guild.name} × {datetime.datetime.now().strftime('%H:%M %d/%m/%Y')}",
...
async def temp(ctx: commands.Context):
    ...
    em_temp_img.set_image(
        url="https://cdn.discordapp.com/attachments/1129601347352809532/1159560783429111838/195fa6c7da1d6a2e.png")
    ...
    em_temp.set_image(
        url="https://cdn.discordapp.com/attachments/1129601347352809532/1166231336885948457/yacherumade1.png")
